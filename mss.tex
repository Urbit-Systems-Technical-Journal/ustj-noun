...
\begin{table}[ht]
\centering
\caption{Example atom encodings with length information appended as trailing zeros (unnamed encoding).  Elided digits are shown in light gray for clarity.}
\label{tab:atom-encodings}
\begin{tabular}{ l l r }
  \textbf{Atom} $\bm{a}$ & \textbf{Length (bits)} & \multicolumn{1}{l}{\textbf{Encoded Form} $\bm{\bar{a}}$} \\
  \hline
  \texttt{0b0} & 1 & \texttt{0b\textcolor{lightgray}{0}1} \\
  ...
  \texttt{0x1234} & 13 & \llap{\texttt{0b100100011010010000000000000}} \\
\end{tabular}
\end{table}
...
\begin{tabular}{ l l r }
  ...
  \texttt{0x1234} & 13 & \llap{\texttt{0b\textcolor{lightgray}{100}100011010010000000000000}} \\
  % \texttt{0x1234} & 13 & \texttt{0b\textcolor{lightgray}{100}100011010010000000000000} \\
...
Consider the noun \texttt{[4 [0 1]]} (= \texttt{[4 0 1]}), represented as a binary tree in Figure~\ref{fig:binary-tree-example}.  Reading across in a depth-first traversal, we produce an encoding order:
...
\begin{tabular}{ r r }
  ...
  \hline
  \texttt{0} & \texttt{0b\textcolor{lightgray}{0}.1.0} \\
...
\noindent
Decoding is left as an exercise to the motivated reader.
