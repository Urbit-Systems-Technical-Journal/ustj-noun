...
\newcommand{\authorname}{N. E. Davis \patp{lagrev-nocfep},* Brian Klatt \patp{ticlys-monlun},* \& Sam Parker \patp{borlug-walryd}†}
% \newcommand{\authorpatp}{, , \& \patp{borlug-walryd}}
...
\author{N. E. Davis \patp{lagrev-nocfep},* \\ Brian Klatt \patp{ticlys-monlun},* \\ \& Sam Parker \patp{borlug-walryd}† \\ \affiliation}
\date{}
...
\begin{abstract}
Noun serialization is commonly used for Nock communication, both between instances like Urbit ships and with the runtime and the Unix host operating system.  This article describes and compares the two principal conventions for representing nouns in slightly compressed form as byte arrays, as well as introduces variant encodings for educational purposes.\footnote{Brian Klatt contributed the description of \texttt{+jam}.  Sam Parker contributed the description of \texttt{+bulk}.  We gratefully acknowledge the contributions of the original architects of \texttt{+jam}/\texttt{+cue} as well.}
\end{abstract}
...
\begin{tabular}{ l l r }
  \textbf{Atom} $a$ & \textbf{Length (bits)} & \multicolumn{1}{l}{\textbf{Encoded Form} $\bar{a}$} \\
  \hline
...
\noindent
We can make one further optimization:  since the most significant bit (\textsc{msb}) of the atom is always \texttt{1} except in the case of zero (which is \texttt{0b10} with the leading zero trimmed), we can omit it from the encoded form.  This saves one bit for every non-zero atom.  Let's call this scheme \texttt{+fat} encoding.  (See Table~\ref{tab:fat-encodings} for examples.)
...
\begin{table}[ht]
\centering
\caption{Example atom encodings (\texttt{+fat} encoding).}
\label{tab:fat-encodings}
\begin{tabular}{ l l r }
  \textbf{Atom} $a$ & \textbf{Length (bits)} & \multicolumn{1}{l}{\textbf{Encoded Form} $\bar{a}$} \\
  \hline
  \texttt{0b0} & 1 & \texttt{0b1} \\
  \texttt{0b1} & 1 & \texttt{0b10} \\
  \texttt{0b10} & 2 & \texttt{0b100} \\
  \texttt{0b11} & 2 & \texttt{0b1100} \\
  \texttt{0b101} & 3 & \texttt{0b11000} \\
  \texttt{0b111} & 3 & \texttt{0b111000} \\
  \texttt{0x70} & 7 & \texttt{0b11000010000000} \\
  \texttt{0xff} & 8 & \texttt{0b1111111100000000} \\
\end{tabular}
\end{table}

The primary problem with the \texttt{+fat} scheme is that the encoding of an atom is twice the bit length of the original atom.  The next optimization is to use the bit length of the bit length, which is logarithmic in the bit length.
...
\begin{tabular}{ r c c r }
  \multicolumn{1}{l}{\textbf{Atom}} $a$ & \textbf{Length (bits)} & \textbf{Mathematics} & \multicolumn{1}{l}{\textbf{Encoded Form} $\bar{a}$} \\
  \hline
...
With \texttt{+mat} encoding in hand, we can produce a proper serialization scheme for nouns by combining the type bit with the encoded atom or recursively encoding cells.  We call this scheme \texttt{+gel} encoding.
...
Consider the noun \texttt{[a [b c]]}, represented as a binary tree in Figure~\ref{fig:binary-tree-example}.  Reading across in a depth-first traversal, we produce an encoding order:
...
\begin{tabular}{ r r }
  ...
  \texttt{[1 0]} & \texttt{0b110.0.1.0.1} \\
  \texttt{[2 1 0]} & \texttt{0b110.0.1.0.1.100100.0.1} \\
  \texttt{[[2 3] 1 0]} & \texttt{0b110.0.1.0.1.110100.0.100100.0.1.1} \\
\end{tabular}
\end{table}
...
A Python implementation of \texttt{+jam} is included in Appendix~A.
...
Likewise, a Python implementation of \texttt{+cue} is included in Appendix~A.
...
A directed graph encoding has been independently proposed twice, once by Tlon in the original Hoon codebase as a \lstinline[style=inlinecode]{+$silo TODO} encoding and once by Sam Parker as the \lstinline[style=inlinecode]{+bulk} encoding.
  ...
  \centering
    ...
    \begin{subfigure}{\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[vertex, very thick] (Root) at (1.5,4.5) {};
        \node[vertex, double, thick] (A) at (0,3) {};
        \node[vertex, dashed, thick] (B) at (3,3) {};
        \node[vertex, dotted, thick] (C) at (0,0) {};
        \node[vertex, thick] (D) at (3,0) {};

        \draw[->, very thick] (Root) -- node[above, draw=none] {L} (A);
        \draw[->, very thick] (Root) -- node[above, draw=none] {R} (B);

        \draw[->, double, thick] (A) -- node[above, draw=none] {L} (B);
        \draw[->, double, thick] (A) -- node[left, draw=none] {R} (C);
...
\subsection*{\patp{fodwyt-ragful}}

The following is a simple Python implementation of \texttt{+jam} serialization drawn from Urbit’s auxiliary \texttt{pynoun} library.\footnote{The original \texttt{pynoun} library was authored by Paul Driver \patp{fodwyt-ragful}.}
